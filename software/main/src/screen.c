...
void screen_update_handler_task(void *arg)
{
    ...
    screen_handle_t screen_handle = (screen_handle_t)arg;
    ...
    while (true)
    {
        ...
        {
            ssd1306_clear_screen(&(screen_handle->dev), false);
            vTaskDelay(pdMS_TO_TICKS(10));
            continue;
        }

        // Set contrast accordingly
        ssd1306_contrast(&(screen_handle->dev), is_screen_dim() ? 0x00 : 0xFF);
        ...
        for (int page = 0; page < 8; page++)
        {
            if (INVERT_SCREEN)
            {
                ssd1306_display_image(&(screen_handle->dev), page, 96, &(screen_handle->segment_image_signatures)[index_array[0] + page * 32], 32);
                ssd1306_display_image(&(screen_handle->dev), page, 70, &(screen_handle->segment_image_numbers)[index_array[1] + page * 32], 32);
                ssd1306_display_image(&(screen_handle->dev), page, 35, &(screen_handle->segment_image_numbers)[index_array[2] + page * 32], 32);
                ssd1306_display_image(&(screen_handle->dev), page, 0, &(screen_handle->segment_image_numbers)[index_array[3] + page * 32], 32);
            }
            else
            {
                ssd1306_display_image(&(screen_handle->dev), page, 0, &(screen_handle->segment_image_signatures)[index_array[0] + page * 32], 32);
                ssd1306_display_image(&(screen_handle->dev), page, 26, &(screen_handle->segment_image_numbers)[index_array[1] + page * 32], 32);
                ssd1306_display_image(&(screen_handle->dev), page, 61, &(screen_handle->segment_image_numbers)[index_array[2] + page * 32], 32);
                ssd1306_display_image(&(screen_handle->dev), page, 96, &(screen_handle->segment_image_numbers)[index_array[3] + page * 32], 32);
            }
        }
...
esp_err_t setup_screen_handler(screen_handle_t screen_handle)
{
    ...
    screen_handle->segment_image_numbers = (uint8_t *)malloc(NUMBER_IMAGES * 8 * 32);
    screen_handle->segment_image_signatures = (uint8_t *)malloc(SIGNATURE_IMAGES * 8 * 32);
    // segment_image_standby = (uint8_t *)malloc(STANDBY_IMAGES * 8 * 32);

    // If memory allocation fails, handle error.
    if (screen_handle->segment_image_numbers == NULL || screen_handle->segment_image_signatures == NULL) // || segment_image_standby == NULL)
    {
    ...
    i2c_master_init(&(screen_handle->dev), SSD1306_SDA_PIN, SSD1306_SCL_PIN, SSD1306_RST_PIN);
    ssd1306_init(&(screen_handle->dev), 128, 32);
    ssd1306_contrast(&(screen_handle->dev), 0xff);
    ssd1306_clear_screen(&(screen_handle->dev), false);

    esp_err_t ret;
    if (INVERT_SCREEN)
    {
        ret = conver_bitmap_to_image(segment_display_numbers_inverse, screen_handle->segment_image_numbers, NUMBER_IMAGES, screen_handle->dev);
    }
    else
    {
        ret = conver_bitmap_to_image(segment_display_numbers, screen_handle->segment_image_numbers, NUMBER_IMAGES, screen_handle->dev);
    }
    if (ret != ESP_OK)
    {
    ...
    if (INVERT_SCREEN)
    {
        ret = conver_bitmap_to_image(segment_display_signatures_inverse, screen_handle->segment_image_signatures, SIGNATURE_IMAGES, screen_handle->dev);
    }
    else
    {
        ret = conver_bitmap_to_image(segment_display_signatures, screen_handle->segment_image_signatures, SIGNATURE_IMAGES, screen_handle->dev);
    }
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Segment image conversion failed.");
...
{
    // Create tag
    static const char *TAG = "start_screen_handler";
    ESP_LOGI(TAG, "Starting the screen handler.");

    // Setup task parameters and start the task
    if (screen_handle->update_screen_task == NULL)
    {
        BaseType_t x_returned;
        x_returned = xTaskCreate(screen_update_handler_task, "screen_update_handler", 2048, (void *)screen_handle, 10, &(screen_handle->update_screen_task));
...
{
    // Create tag
    static const char *TAG = "stop_screen_handler";
    ESP_LOGI(TAG, "Turning of screen handler.");
